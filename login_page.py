...
class login_page:
    def __init__(self, root):
        ...
        label1 = Label(self.frame1, text="Life", font=("times new roman", 40, "bold"), bg="yellow", fg="red").place(
            x=50, y=300)
        label2 = Label(self.frame1, text="Choices", font=("times new roman", 40, "bold"), bg="yellow",
                       fg="RoyalBlue1").place(x=162, y=300)
        label3 = Label(self.frame1, text="Registration Form", font=("times new roman", 13, "bold"), bg="yellow",
    ...
    def login_func(self):
        if self.email_entry.get() == "" or self.password_entry.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host="127.0.0.1", user="lifechoices", password="@Lifechoices1234",
                                             database="LifeChoicesOnline", auth_plugin='mysql_native_password')
                cur = connection.cursor()
                cur.execute("select * from Userdetails where EmailID=%s and Password=%s",
                            (self.email_entry.get(), self.password_entry.get()))
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error!", "Invalid USERNAME & PASSWORD", parent=self.window)
                else:
                    messagebox.showinfo("Success", "Wellcome to the Life Choices", parent=self.window)
        ...
        if self.email_entry.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host="127.0.0.1", user="lifechoices", password="@Lifechoices1234",
                                             database="LifeChoicesOnline", auth_plugin='mysql_native_password')
                cur = connection.cursor()
                cur.execute("select * from Userdetails where EmailID=%s", self.email_entry.get())
                row = cur.fetchone()
                if row == None:
                    ...
                    self.create_button = Button(self.root, text="Submit", command=self.change_pass,
                                                ...
                                                bg="green2", fg="white").place(x=95, y=340, width=200)

                    self.create_button = Button(self.root, text="Admin", command=self.Admin_page,
                                                font=("times new roman", 18, "bold"), bd=0, cursor="hand2",
                                                bg="green2", fg="white")
                    self.create_button.bind(self.root, '<Ctrl-A>', self.Admin_page)
                    self.create_button.place(x=95, y=390, width=200)
    ...
    def change_pass(self):
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host="127.0.0.1", user="lifechoices", password="@Lifechoices1234",
                                             database="LifeChoicesOnine", auth_plugin='mysql_native_password')
                cur = connection.cursor()
                cur.execute("SELECT * FROM Userdetails WHERE EmailID=%s AND question=%s and answer=%s",
